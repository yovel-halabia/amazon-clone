    ...
    return (
        <div className="header">
            ...
            <div className="header__dliver-to header__button app__button">
                <div id="header__map-icon">
                </div>
                <div className="header__text">
                    <span className="header__text-line1">Deliver to</span>
                    <span data-testid='headerCountry' className="header__text-line2">{user? user.country: 'Israel'}</span>
                </div>
            </div>
            }
            ...
            <div className=' header__button app__button'>
                ...
                <Link to={!user&&"/login"} onClick={user&&handleSignOut}>
                    <div className='header__account header__text' >
                        <span data-testid='headerUserName' className="header__text-line1">Hello, {user ?  user.email.split('@')[0] : 'Guest' }</span>
                        <span className="header__text-line2">{user?'Sign Out':'Sign In'}</span>
                    </div>
                </Link>
                :
                <div  onClick={()=>{handleShowMenu(true);document.body.style.overflow = "hidden";}} >
                ...
                <Link to="/checkout">
                   <div className="header__cart">
                        <div className='header__cart header__icon-cart'>
                            <span data-testid="headerCartAmount" id="header__cart-amaount">{getBasketLength(basket)}</span>
                        </div>
                        {!isMobie&&
                        <span>Cart</span>
                        }
